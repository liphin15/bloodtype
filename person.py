...
class Person:
    ...
    def genOffspring(self, otherPerson):
        # implement gene selection from self and otherPerson
        # print("############### {} - {} ###############".format(self.sex, otherPerson.sex))
        child_genotype = (self.genotype[getrandbits(1)], otherPerson.genotype[getrandbits(1)])
        child_rf_genotype = (self.rf_genotype[getrandbits(1)], otherPerson.rf_genotype[getrandbits(1)])
    ...
    def mutateGenome(self, mutateTo):
        self.genotype = (self.genotype[0], mutateTo) if bool(getrandbits(1)) else (mutateTo, self.genotype[1])
        self.setPhenotype()

    def mutateRfGenome(self, mutateTo):
        self.rf_genotype = (self.rf_genotype[0], mutateTo) if bool(getrandbits(1)) else (mutateTo, self.rf_genotype[1])
        self.setRfPhenotype()
    ...
    def setRfPhenotype(self):
        if self.rf_genotype == ('+', '+') or self.rf_genotype == ('-', '+') or self.rf_genotype == ('+', '-'):
            self.rf_phenotype = '+'
