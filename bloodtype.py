...
class BloodType:
    ...
    def step(self, size=1):
        ...
        phenoptypeCounts = [phenotypeFromPersons.count(p) for p in self.phenotypes]
        self.state.append([
            size,
            np.unique([i.sex for i in self.persons], return_counts=True)[1],
            phenoptypeCounts,
        ])
    ...
    def checkPerson(self, i_person):
        ...
        if self.persons[i_person].possibleOffspring():
            ...
            if ('f' not in sexs) or ('m' not in sexs):
                ...
                return None
            j_person = np.random.choice(np.arange(len(self.persons))[sexs != self.persons[i_person].sex],1)[0]
    ...
    def genMutatedOffspring(self, genotypeMutation=None, rfGenotypeMutation=None):
        i_person = random.randint(0, len(self.persons) - 1)
        sexs = np.array([p.sex for p in self.persons])
        j_person = np.random.choice(np.arange(len(self.persons))[sexs != self.persons[i_person].sex],1)[0]
        child = self.persons[i_person].genOffspring(self.persons[j_person])
    ...
    def plot_phenotype(self, ratio=False):
        ...
        for i, p in enumerate(self.phenotypes):
            ...
            ax.fill_between(x, y[:, i], np.zeros(y[:, 0].shape) if i == 0 else y[:, i-1],
                            color=self.phenotypeColors[p], alpha=1, label=p)
        plt.legend()
        plt.show()

    def plot_sex(self, ratio=False):
        x = np.cumsum([entry[0] for entry in self.state])
        ...
        ax.fill_between(x, y[:,0], np.zeros(y[:,0].shape), color=default_colors[1], alpha=1, label='f')
        ax.fill_between(x, y[:,1], y[:,0], color=default_colors[0], alpha=1, label='f+m')
        plt.legend()
        plt.show()
